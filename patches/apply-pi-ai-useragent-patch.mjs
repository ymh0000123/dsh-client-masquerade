...
//   node patches/apply-pi-ai-useragent-patch.mjs            # apply (default)
//   node patches/apply-pi-ai-useragent-patch.mjs --revert    # revert to stock
...
//
// The OLD/NEW/MARKER strings and the apply/revert logic live in patch-lib.js
// (shared with the installed Host plugin so the web settings page can apply and
// revert the same patch).
...
const { applyPatch, revertPatch } = require('./patch-lib.js');
...
try {
  if (process.argv.indexOf('--revert') >= 0) {
    const result = revertPatch(target);
