...
// Localized card dashboard (Settings -> Client Masquerade) + run-card panel.
...
const DICTS = {
  en: {
    ...
    identityHelp: 'Writes spoofed client-identity headers into the selected provider profile; they are sent on every request to that route. The reserved attribution user-agent cannot be overridden, and the headers only reach the wire once the User-Agent patch is applied.',
    ...
    pUnknown: 'unknown',
    uaUnsupported: 'A dynamic plugin cannot inspect or write these patches. Run the scripts under node_modules/dsh-client-masquerade/patches/ manually (apply-pi-ai-useragent-patch.mjs, apply-pi-ai-body-patch.mjs, apply-variant-retry-patch.mjs), then restart dsh web. Installing the package instead makes them one-click here.',
  ...
  zh: {
    ...
    patchMixed: '部分应用',
    cardIdentity: '身份伪装',
    identityHelp: '把伪造的客户端身份头写入所选 provider 配置，该路由的每次请求都会携带。保留的归属 user-agent 无法覆盖；只有应用 User-Agent 补丁后，这些头才会真正上线。',
    ...
    pSkipped: '未安装',
    pUnknown: '未知',
    uaUnsupported: '动态插件无法检查或写入这些补丁。请手动执行 node_modules/dsh-client-masquerade/patches/ 下的脚本（apply-pi-ai-useragent-patch.mjs、apply-pi-ai-body-patch.mjs、apply-variant-retry-patch.mjs），然后重启 dsh web。改为安装该包即可在此一键操作。',
    cardHeaders: '当前请求头',
    headersCount: '{n}',
    noHeaders: '该路由无额外请求头'
  }
...
return {
  apply(ctx) {
    ...
    function MaskPanel() {
      const [state, setState] = React.useState({ providers: [], selected: '', busy: false, message: '', error: '', uaPatch: null, variantPatch: null, bodyPatch: null, sentinelTools: [], queueBusy: false, bodyBusy: false, testing: false });
      ...
      const label = () => {
        const found = state.providers.find((p) => p.id === state.selected);
        return found && found.displayName ? found.displayName : state.selected;
      };
      ...
      const anyBusy = state.busy || state.testing || state.bodyBusy || state.queueBusy;
      ...
      const sentinel = state.sentinelTools.length > 0 ? state.sentinelTools : ['Glob', 'Grep', 'Read'];
      const bodyCard = el('div', { className: 'cmq-card' },
        ...
        el('div', { className: 'cmq-card-body' },
          ...
          el('div', { className: 'cmq-help' }, t('sentinelHelp'))
        )
      );
      ...
      // Dynamic mode cannot read or write the patch files, so this card is a
      // read-only status list plus the manual instructions.
      const patchRows = [
